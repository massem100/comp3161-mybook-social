...
def dummy_data(x):
    fake_data={}
    for i in range(0,x):
        g=fake.random_choices(elements=[('male'),('female')],length=1)
        #m=fake.name_male()
        #f=fake.name_female()
        if g =='male':
           a=fake.first_name_male()
        else:
           a=fake.first_name_female()
        fake_data[i]={}
        fake_data[i]['User ID']=fake.bothify(text='U?-###')
        fake_data[i]['First Name']=a
        fake_data[i]['Last Name']=fake.last_name()
        ...
        fake_data[i]['Group Name']=fake.company()
        ...
        fake_data[i]['Date Poster']=fake.date_this_century()
